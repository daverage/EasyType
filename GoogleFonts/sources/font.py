...
def sanitize_stat_table(tt):
    ...
    print(f"✓ STAT table reduced to {len(filtered)} axis values (removed {len(axis_values) - len(filtered)})")
...
def _draw_as_contours(tt, glyph_name, pen, transform=IDENTITY_TRANSFORM):
...
def flatten_composites(tt):
    ...
    for glyph_name in tt.getGlyphOrder():
        ...
        new_glyph = pen.glyph()
        if hasattr(glyph, "program") and getattr(glyph, "program", None):
